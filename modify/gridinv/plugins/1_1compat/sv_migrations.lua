...
function MODULE:migrateBagSize(res)
	...
	local ID_MATCH = lia.db.escape(util.TableToJSON({id = invID}):sub(2, -2))
	local CONDITION = "_data LIKE '%"..ID_MATCH.."%'"
	return lia.db.select(ITEM_FIELDS, ITEMS_TABLE, CONDITION, 1)
...
function MODULE:migrateInventorySize(res)
	local w, h = lia.config.get("invW"), lia.config.get("invH")
	if (isnumber(w) and isnumber(h)) then
		local addW = self:addInventoryData(res, "w", w)
		local addH = self:addInventoryData(res, "h", h)
		return deferred.all({addW, addH}):next(function()
...
function MODULE:migrateInventories()
	...
	lia.db.select(FIELDS, INV_TABLE, self:getMigrationFilter())
		:next(function(res)
			local migrations = {}
