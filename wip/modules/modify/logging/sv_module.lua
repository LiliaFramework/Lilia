...
function MODULE:OnTakeShipmentItem(client, itemClass, amount)
    local itemTable = lia.item.list[itemClass]
    lia.log.add(client, "shipment", itemTable.name)
end

function MODULE:OnCreateShipment(client, shipmentEntity)
    lia.log.add(client, "shipmentO")
end

function MODULE:OnCharTradeVendor(client, vendor, x, y, invID, price, isSell)
end
