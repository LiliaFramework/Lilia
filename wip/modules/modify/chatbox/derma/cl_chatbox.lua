local PANEL = {}
	...
	function PANEL:setActive(state)
		...
		if (state) then
			...
			self.text.OnEnter = function(this)
				...
				if (text:find("%S")) then
					if (!(lia.chat.lastLine or ""):find(text, 1, true)) then
			...
			self.text.Paint = function(this, w, h)
				...
				this:DrawTextEntryText(TEXT_COLOR, CONFIG.Color, TEXT_COLOR)
			end
	...
	local function PaintFilterButton(this, w, h)
		if (this.active) then
			...
			surface.SetDrawColor(ColorAlpha(CONFIG.Color, alpha))
		end
			...
			elseif (type(v) == "Player") then
				...
				text = text:gsub("%b**", function(value)
					...
					if (inner:find("%S")) then
						return "<font=liaChatFontItalics>"..value:sub(2, -2).."</font>"
					end
				end)
			end
		end
