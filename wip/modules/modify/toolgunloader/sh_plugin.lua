local MODULE = MODULE
MODULE.name = "Toolgun Tool Loader"
MODULE.desc = "Allows modules to easily load new tools for the toolgun."
MODULE.ToolObj = MODULE.ToolObj or {}
local ToolObj = MODULE.ToolObj
...
function MODULE:InitializedModules()
    for id, module in next, lia.module.list do
        self:LoadTools(id)
    end
...
function MODULE:LoadTools(moduleID)
    local module = lia.module.list[moduleID]
    local files = file.Find(module.path .. "/stools/*.lua", "LUA")

    for _, tool in ipairs(files) do
        local char1, char2, toolmode = string.find(tool, "([%w_]*).lua")
        TOOL = ToolObj:Create()
        TOOL.Mode = toolmode
        AddCSLuaFile(module.path .. "/stools/" .. tool)
        include(module.path .. "/stools/" .. tool)
        TOOL:CreateConVars()
