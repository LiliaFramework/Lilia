...
function MODULE:whereSameID(res)
	return "_invID = " .. lia.db.escape(tostring(res._invID))
end

function MODULE:getMigrationFilter()
	local typeIDs = table.GetKeys(lia.inventory.types)
	local escapedTypeIDs = {}

	for i, typeID in ipairs(typeIDs) do
		escapedTypeIDs[i] = "'" .. lia.db.escape(typeID) .. "'"
	end

	return "_invType NOT IN (" .. table.concat(escapedTypeIDs, ",") .. ")" .. " or _invType IS NULL"
end

function MODULE:addInventoryData(res, key, value)
...
function MODULE:migrateStorageSize(res)
	STORAGE_DEFINITIONS = STORAGE_DEFINITIONS or {}
	local storage

	for _, storageInfo in pairs(STORAGE_DEFINITIONS) do
		if storageInfo.invType == res._invType or storageInfo.legacyInvType == res._invType then
			storage = storageInfo
			break
		end
	end
	...
	if isnumber(w) and isnumber(h) then
		...
		return deferred.all({addW, addH}):next(function()
			self:print("\tMigrated storage inventory " .. res._invID .. " with" .. " (w,h) = (" .. tostring(w) .. "," .. tostring(h) .. ")")
		end)
	end
end

function MODULE:deleteCharID(res)
	-- TODO: add promise support for lia.db.*Table
	local d = deferred.new()
	...
	lia.db.updateTable({
		...
		d:resolve()
	end, INV_TABLE, "_invID = " .. res._invID)

	return d
end

function MODULE:migrateBagSize(res)
	...
	local ID_MATCH = lia.db.escape(util.TableToJSON({
		id = invID
	}):sub(2, -2))

	local CONDITION = "_data LIKE '%" .. ID_MATCH .. "%'"

	return lia.db.select(ITEM_FIELDS, ITEMS_TABLE, CONDITION, 1):next(function(queryResults)
		if not queryResults or not queryResults.results then return end
		...
		return deferred.all({self:addInventoryData(res, "item", itemID), self:addInventoryData(res, "w", w), self:addInventoryData(res, "h", h), self:deleteCharID(res)}):next(function()
			self:print("\tMigrated bag inventory for item " .. itemID .. " with (w,h) = (" .. tostring(w) .. "," .. tostring(h) .. ")")
		end)
	end)
end
...
if isnumber(w) and isnumber(h) then
	...
	return deferred.all({addW, addH}):next(function()
		self:print("\tMigrated player inventory " .. res._invID .. " with" .. " (w,h) = (" .. tostring(w) .. "," .. tostring(h) .. ")")
	end)
end
...
function MODULE:migrateInventory(res)
...
function MODULE:migrateInventories()
	...
	lia.db.select(FIELDS, INV_TABLE, self:getMigrationFilter()):next(function(res)
		local migrations = {}
		...
		hook.Add("ShouldDataBeSaved", "liaTemporarySession", function()
			return false
		end)

		self:print("FINISHED MIGRATIONS")
		local hibernationBool = lia.data.get("currentHibernationBool", 0, true, true)
...
concommand.Add("lia_migrateinv", function(client)
	if IsValid(client) then return end
	MODULE:migrateInventories()
end)