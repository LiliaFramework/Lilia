...
if SERVER then
    ...
    function PANEL:Init()
        ...
        function toggleZ:DoClick()
            HELPER_INFO.ignoreZ = not HELPER_INFO.ignoreZ
        end
        ...
        function cpyInfo:DoClick()
            SetClipboardText(Format([[Local Pos, Ang:
				Pos = Vector(%s, %s, %s)
				Ang = Angle(%s, %s, %s)]], HELPER_INFO.renderPos[1], HELPER_INFO.renderPos[2], HELPER_INFO.renderPos[3], HELPER_INFO.renderAng[1], HELPER_INFO.renderAng[2], HELPER_INFO.renderAng[3]))
        end
    end

    vgui.Register("liaHelperFrame", PANEL, "DFrame")
    ...
    function SWEP:Reload()
        if not self.menuOpen then
            self.menuOpen = true
            local a = vgui.Create("liaHelperFrame")
    ...
    function SWEP:DrawHUD()
        local w, h = ScrW(), ScrH()
        local cury = h / 4 * 3
        local tx, ty = draw.SimpleText("Left Click: Select Entity", "liaMediumFont", w / 2, cury, color_white, 1, 1)
        cury = cury + ty
        local tx, ty = draw.SimpleText("Right Click: Deselect Entity", "liaMediumFont", w / 2, cury, color_white, 1, 1)
        cury = cury + ty
        local tx, ty = draw.SimpleText("Reload: Register Area", "liaMediumFont", w / 2, cury, color_white, 1, 1)
    end
