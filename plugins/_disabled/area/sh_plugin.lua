...
ALWAYS_RAISED["lia_areahelper"] = true

lia.config.add("areaFontSize", 26, "The size of the font of Area Display.",
	function(oldValue, newValue)
		if (CLIENT) then
			hook.Run(
				"LoadLiliaFonts",
				lia.config.get("font"),
				lia.config.get("genericFont")
			)
		end
	end,
	{data = {min = 1, max = 128},
...
lia.config.add("areaDispSpeed", 20, "The Appearance Speed of Area Display.", nil, {
	data = {min = 1, max = 40},
	category = "areaPlugin"
})
...
if (SERVER) then
	function lia.area.getArea(areaID)
		return PLUGIN.areaTable[areaID]
	end

	function lia.area.getAllArea()
	...
	function playerMeta:isInArea(areaID)
		local areaData = lia.area.getArea(areaID)
	...
	function PLUGIN:createAreaTimer()
		timer.Create("liaAreaController", 0.33, 0, function()
			for k, v in ipairs(player.GetAll()) do
				local char = v:getChar()

				if (char and v:Alive()) then
					local area = v:getArea()
					for id, areaData in pairs(lia.area.getAllArea()) do
	...
	-- add area.
	function lia.area.addArea(name, vector1, vector2, desc)
	...
	netstream.Hook("areaEdit", function(client, areaID, editData)
		...
		local areaData = table.Copy(lia.area.getArea(areaID))
	...
	netstream.Hook("areaAdd", function(client, name, vector1, vector2)
		...
		lia.area.addArea(name, vector1, vector2)
	end)
	...
	function lia.area.openAreaManager()
	...
	netstream.Hook("areaManager", function()
		--lia.area.openAreaManager()
	end)

	function PLUGIN:LoadFonts(font)
		timer.Simple(0, function()
			surface.CreateFont("liaAreaDisplay", {
				font = font,
				extended = true,
				size = ScreenScale(lia.config.get("areaFontSize")),
				weight = 500,
				shadow = true,
			})
		end)
	...
	function displayScrText(str, time)
		speed = lia.config.get("areaDispSpeed")
		targetScale = 1
		dispString = str
		dieTime = time or 5
	...
	function PLUGIN:HUDPaint()
		...
		surface.SetFont("liaAreaDisplay")
...
lia.command.add("areaadd", {
	...
	onRun = function(client, arguments)
		...
		if (!client:getNetVar("areaMin")) then
			if (!name) then
				lia.util.Notify(lia.lang.Get("missing_arg", 1), client)
			...
			lia.area.addArea(name, min, max)

			if (SERVER and !timer.Exists("liaAreaController")) then
...
lia.command.add("arearemove", {
	...
	onRun = function(client, arguments)
		...
		if (areaData) then
			...
			if (SERVER and #lia.area.getAllArea() < 1) then
				timer.Remove("liaAreaController")
			end
...
lia.command.add("areamanager", {
	adminOnly = true,
	onRun = function(client, arguments)
		if (client:Alive()) then
			netstream.Start(client, "liaAreaManager", lia.area.getAllArea())
		end
	end
})
