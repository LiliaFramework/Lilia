...
local function CanAccessInventoryIfCharacterIsOwner(inventory, action, context)
	...
	if (table.HasValue(client.liaCharList, ownerID)) then
		return true
	end
end
...
if (SERVER) then
	function SimpleInv:add(itemTypeOrItem, quantity, forced)
		...
		for i = 1, quantity do
			lia.item.instance(self:getID(), itemType, nil, 0, 0, function(item)
				self:addItem(item)
				items[#items + 1] = item
...
function SimpleInv:getMaxWeight()
	local maxWeight =
		self:getData("maxWeight", lia.config.get("invMaxWeight", MAX_WEIGHT))
	for _, item in pairs(self.items) do
