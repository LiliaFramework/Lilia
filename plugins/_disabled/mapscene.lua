...
if (CLIENT) then
	...
	function PLUGIN:CalcView(client, origin, angles, fov)
		...
		local shouldShow = IsValid(lia.gui.character)
...
lia.command.add("mapsceneadd", {
	...
	onRun = function(client, arguments)
		...
		if (util.tobool(arguments[1]) and !client.liaScnPair) then
			client.liaScnPair = {position, angles}

			return L("mapRepeat", client)
		else
			if (client.liaScnPair) then
				PLUGIN:addScene(client.liaScnPair[1], client.liaScnPair[2], position, angles)
