local gradient = lia.util.getMaterial("vgui/gradient-r.vtf")
local glow = surface.GetTextureID("particle/Particle_Glow_04_Additive")
local PANEL = {}

function PANEL:Init()
    if IsValid(lia.gui.intro) then
        lia.gui.intro:Remove()
    end
    ...
    self.authors:SetFont("liaIntroMediumFont")
    ...
    self.name:SetFont("liaIntroTitleFont")
    ...
    self.schema:SetFont("liaIntroBigFont")
...
function PANEL:addContinue()
    ...
    self.info:SetFont("liaIntroSmallFont")
...
function PANEL:Think()
    if IsValid(self.info) and input.IsKeyDown(KEY_SPACE) and not self.closing then
        self.closing = true
...
vgui.Register("liaIntro", PANEL, "EditablePanel")