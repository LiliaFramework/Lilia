timer.Create("liaLifeGuard", 1, 0, function()
	if (not lia.config.get("drownEnabled")) then return end
	for k, v in ipairs(player.GetAll()) do
		if (not v:getChar() or not v:Alive()) then continue end
		if (hook.Run("ShouldPlayerDrown", v) == false) then continue end
		local damage = lia.config.get("drownDamage")

		if (v:WaterLevel() >= 3) then
			if (!v.drowningTime) then
				v.drowningTime = CurTime() + lia.config.get("drownTime")
