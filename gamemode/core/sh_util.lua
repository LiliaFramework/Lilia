...
function lia.util.getAddress()
	ErrorNoHalt(
		"lia.util.getAddress() is deprecated, use game.GetIPAddress()\n"
	)
...
function lia.util.findPlayer(identifier, allowPatterns)
	if (lia.util.isSteamID(identifier)) then
...
do
	...
	if (SERVER) then
		function playerMeta:getPlayTime()
			local diff = os.time(lia.util.dateToNumber(self.lastJoin))
				- os.time(lia.util.dateToNumber(self.firstJoin))
			return diff + (RealTime() - (self.liaJoinTime or RealTime()))
		end
	else
		function playerMeta:getPlayTime()
			local diff = os.time(lia.util.dateToNumber(lia.lastJoin))
				- os.time(lia.util.dateToNumber(lia.firstJoin))
	...
	function playerMeta:isFemale()
		...
		return model:find("female")
			...
			or lia.anim.getModelClass(model) == "citizen_female"
	end
...
function lia.util.getMaterial(materialPath)
	-- Cache the material.
	lia.util.cachedMaterials[materialPath] =
		lia.util.cachedMaterials[materialPath]
