...
net.Receive("liaKeybindServer", function(_, ply)
    ...
    if isRelease then
        if data.release and data.serverOnly then
            ...
            if not success then print(L("keybindReleaseError", tostring(player), tostring(err))) end
        end
    else
        if data.callback and data.serverOnly then
            local success, err = pcall(data.callback, player)
            if not success then print(L("keybindPressError", tostring(player), tostring(err))) end
        end
    end
end)
