...
hook.Add("PopulateConfigurationButtons", "liaOptionsPopulate", function(pages)
    local OptionFormatting = {
        Int = function(key, name, cfg, parent)
            ...
            local p = c:Add("liaBasePanel")
            p:Dock(FILL)
            p.Paint = nil
            local lbl = p:Add("liaText")
            lbl:Dock(TOP)
            lbl:SetTall(45)
            lbl:SetText(name)
            ...
            local slider = p:Add("liaSlideBox")
            slider:Dock(FILL)
        ...
        Color = function(key, name, cfg, parent)
            ...
            btn.DoClick = function()
                ...
                frm:Center()
                frm:MakePopup()
                local mixer = frm:Add("liaColorMixer")
                mixer:Dock(FILL)
                mixer:SetPalette(true)
                mixer:SetAlphaBar(true)
                mixer:SetWangs(true)
                mixer:SetColor(lia.option.get(key, cfg.value))
                local apply = frm:Add("liaButton")
                apply:Dock(BOTTOM)
                apply:SetTall(40)
    ...
    local function buildOptions(parent, filter)
        ...
        for _, catName in ipairs(catNames) do
            ...
            local cat = vgui.Create("liaCategory", parent)
            cat:Dock(TOP)
