...
--[[
    Purpose: Registers a new configurable option in the Lilia framework with automatic type detection and UI generation
    When Called: During module initialization or when adding new user-configurable settings
    Parameters:
...
--[[
    Purpose: Retrieves the available options for a dropdown/selection type option, handling both static and dynamic option lists
    When Called: When rendering dropdown options in the UI or when modules need to access option choices
    Parameters:
...
--[[
    Purpose: Sets the value of an option, triggers callbacks, saves to file, and optionally networks to clients
    When Called: When user changes an option value through UI or when programmatically updating option values
    Parameters:
...
--[[
    Purpose: Retrieves the current value of an option, falling back to default value or provided fallback if not set
    When Called: When modules need to read option values for configuration or when UI needs to display current values
    Parameters:
...
--[[
    Purpose: Saves all current option values to a JSON file for persistence across sessions
    When Called: Automatically called when options are changed, or manually when saving configuration
    Parameters: None
    Returns: None
    Realm: Client
    Example Usage:
...
--[[
    Purpose: Loads saved option values from JSON file and initializes options with defaults if no saved data exists
    When Called: During client initialization or when manually reloading option configuration
    Parameters: None
    Returns: None
    Realm: Client
    Example Usage:
