...
--[[
    Purpose: Find a player by their Steam ID or Steam ID 64
    When Called: When you need to locate a specific player using their Steam identification for operations like bans, whitelists, or data retrieval
    Parameters:
...
--[[
    Purpose: Find all players within a specified spherical radius from a center point
    When Called: When you need to find players in a circular area for proximity-based operations like damage, effects, or notifications
    Parameters:
...
--[[
    Purpose: Find a player by various identifier types including Steam ID, Steam ID 64, name, or special selectors
    When Called: When you need to locate a specific player using flexible identification methods for commands, admin actions, or interactions
    Parameters:
...
--[[
    Purpose: Find all items created by a specific player in the world
    When Called: When you need to locate dropped items or spawned entities created by a particular player for cleanup, tracking, or management
    Parameters:
...
--[[
    Purpose: Find all items of a specific class created by a particular player
    When Called: When you need to locate specific types of items created by a player for targeted operations like weapon cleanup or resource management
    Parameters:
        client - Player: The player whose created items should be found
...
--[[
    Purpose: Find all entities created by or associated with a specific player, optionally filtered by class
    When Called: When you need to locate entities spawned by a player for management, cleanup, or tracking purposes
    Parameters:
...
--[[
    Purpose: Check if two strings match using flexible comparison methods including case-insensitive and partial matching
    When Called: When you need to compare strings with flexible matching for search functionality, name validation, or text processing
    Parameters:
...
--[[
    Purpose: Get a list of all currently online administrators/staff members
    When Called: When you need to identify staff members for admin-only operations, notifications, or privilege checks
    Parameters: None
...
--[[
    Purpose: Find a player by their Steam ID 64, converting it to Steam ID format first
    When Called: When you need to locate a player using their Steam ID 64 for database operations or external integrations
    Parameters:
...
--[[
    Purpose: Find a player by their Steam ID
    When Called: When you need to locate a player using their Steam ID for admin actions, bans, or data retrieval
    Parameters:
...
--[[
    Purpose: Check if an entity can fit at a specific position without colliding with solid objects
    When Called: When you need to validate if an entity can be placed at a location for spawning, teleportation, or collision detection
    Parameters:
...
--[[
    Purpose: Find all players within a specified radius from a center position
    When Called: When you need to find players in a circular area for proximity-based operations like damage, effects, or area management
    Parameters:
...
--[[
    Purpose: Format a string using named placeholders with flexible argument handling
    When Called: When you need to format strings with named parameters for localization, templating, or dynamic text generation
    Parameters:
...
--[[
    Purpose: Get a cached material object from a file path, creating it if it doesn't exist
    When Called: When you need to load and cache materials for rendering, UI elements, or visual effects to improve performance
    Parameters:
...
--[[
    Purpose: Find a faction by name or unique ID using flexible matching
    When Called: When you need to locate faction information for player assignment, permissions, or faction-based operations
    Parameters:
...
--[[
    Purpose: Generate a random full name by combining first and last names from provided or default lists
    When Called: When you need to create random character names for NPCs, testing, or procedural content generation
    Parameters:
...
if SERVER then
    --[[
        Purpose: Send a table-based user interface to a specific client for displaying data in a structured format
        When Called: When you need to display tabular data to a player, such as inventories, player lists, or administrative information
        Parameters:
    ...
    --[[
        Purpose: Find empty spaces around an entity for spawning or placement purposes
        When Called: When you need to find valid locations to spawn entities, NPCs, or items around a central position
        Parameters:
    ...
    --[[
        Purpose: Animate a panel's appearance with scaling, positioning, and alpha transitions
        When Called: When you need to create smooth entrance animations for UI panels, menus, or dialog boxes
        Parameters:
    ...
    --[[
        Purpose: Clamp a panel's position to stay within screen boundaries while avoiding UI overlap
        When Called: When you need to ensure menus and panels stay visible and don't overlap with important UI elements like logos
        Parameters:
    ...
    --[[
        Purpose: Draw a gradient background using predefined gradient materials
        When Called: When you need to create gradient backgrounds for UI elements, panels, or visual effects
        Parameters:
    ...
    --[[
        Purpose: Wrap text to fit within a specified width, breaking it into multiple lines
        When Called: When you need to display text that might be too long for a UI element, ensuring it wraps properly
        Parameters:
    ...
    --[[
        Purpose: Draw a blur effect behind a panel using screen-space blurring
        When Called: When you need to create a blurred background effect for UI elements like menus or dialogs
        Parameters:
    ...
    --[[
        Purpose: Draw a black blur effect with enhanced darkness behind a panel
        When Called: When you need to create a darker, more opaque blurred background effect for UI elements
        Parameters:
            panel - Panel: The panel to draw blur behind
    ...
    --[[
        Purpose: Draw a blur effect at specific screen coordinates
        When Called: When you need to apply blur effects to specific screen areas for HUD elements or overlays
        Parameters:
    ...
    --[[
        Purpose: Create a complete table-based UI window for displaying data with interactive features
        When Called: When you need to display tabular data with sorting, actions, and interactive options
        Parameters:
    ...
    --[[
        Purpose: Create and display an options menu with interactive buttons
        When Called: When you need to present a list of options or actions to the user in a popup menu
        Parameters:
    ...
    --[[
        Purpose: Draw floating text above an entity with distance-based fade effects
        When Called: When you need to display information or labels above entities in the 3D world
        Parameters:
    ...
    --[[
        Purpose: Draw floating text at the player's look position with distance-based fade effects
        When Called: When you need to display contextual information at the location the player is looking at
        Parameters:
            text - String: The text to display
            posY - Number: Vertical offset for text positioning (default: 0)
            alphaOverride - Number: Optional alpha override for manual control
