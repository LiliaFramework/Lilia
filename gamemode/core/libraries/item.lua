...
--[[
   Function: lia.item.getInv (Duplicate Notice)

   Description:
      Retrieves an inventory table by its ID from lia.inventory.instances.
      (This appears again, but references lia.inventory.instances directly.)

   Parameters:
      invID (number) - The ID of the inventory to retrieve.
   ...
   Example Usage:
      local inv = lia.item.getInv(7)
      if inv then
          print("Inventory ID 7 found.")
      end
]]
