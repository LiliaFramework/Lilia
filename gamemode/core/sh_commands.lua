timer.Simple(10, function()
    if not SCHEMA_COMMANDS then
        ...
        lia.command.add("reply", {
            syntax = "<string message>",
            onRun = function(client, arguments)
                local target = client.liaLastPM

                if IsValid(target) and (client.liaNextPM or 0) < CurTime() then
                    lia.chat.send(client, "pm", table.concat(arguments, " "), false, {client, target})
        ...
        lia.command.add("setvoicemail", {
            syntax = "[string message]",
            onRun = function(client, arguments)
                local message = table.concat(arguments, " ")

                if message:find("%S") then
                    client:setLiliaData("vm", message:sub(1, 240))

                    return "@vmSet"
                else
                    client:setLiliaData("vm")
        ...
        lia.command.add("flaggive", {
            ...
            onRun = function(client, arguments)
                ...
                if IsValid(target) and target:getChar() then
                    ...
                    lia.util.notifyLocalized("flagGive", nil, client:Name(), target:Name(), flags)
                end
            end
        })
        ...
        lia.command.add("flagtake", {
            ...
            onRun = function(client, arguments)
                ...
                if IsValid(target) and target:getChar() then
                    ...
                    lia.util.notifyLocalized("flagTake", nil, client:Name(), flags, target:Name())
                end
            end
        })
        ...
        lia.command.add("charsetmodel", {
            ...
            onRun = function(client, arguments)
                ...
                if IsValid(target) and target:getChar() then
                    ...
                    target:SetupHands()
                    lia.util.notifyLocalized("cChangeModel", nil, client:Name(), target:Name(), arguments[2])
                end
            end
        })
        ...
        lia.command.add("charsetskin", {
            ...
            onRun = function(client, arguments)
                ...
                if IsValid(target) and target:getChar() then
                    ...
                    lia.util.notifyLocalized("cChangeSkin", nil, client:Name(), target:Name(), skin or 0)
                end
            end
        })
            ...
            onRun = function(client, arguments)
                ...
                if IsValid(target) and target:getChar() then
                    ...
                    if index > -1 then
                        ...
                        lia.util.notifyLocalized("cChangeGroups", nil, client:Name(), target:Name(), arguments[2], value or 0)
                    else
                        return "@invalidArg", 2
                    end
                end
        ...
        lia.command.add("charsetname", {
            ...
            onRun = function(client, arguments)
                ...
                if IsValid(target) and target:getChar() then
                    lia.util.notifyLocalized("cChangeName", nil, client:Name(), target:Name(), targetName)
        ...
        lia.command.add("chargiveitem", {
            ...
            onRun = function(client, arguments)
                ...
                if IsValid(target) and target:getChar() then
                    ...
                    if (arguments[3] and arguments[3] ~= "") and (not amount) then return L("invalidArg", client, 3) end

                    target:getChar():getInv():add(uniqueID, amount or 1):next(function(res)
                        if IsValid(target) then
        ...
        lia.command.add("charban", {
            ...
            onRun = function(client, arguments)
                ...
                if IsValid(target) then
                    ...
                    if char then
                        lia.util.notifyLocalized("charBan", client:Name(), target:Name())
                        char:ban()
                    end
                end
            end
        ...
        lia.command.add("plytransfer", {
            ...
            onRun = function(client, arguments)
                ...
                if not IsValid(target) or not character then return "@plyNotExist" end
                -- Find the specified faction.
                local oldFaction = lia.faction.indices[character:getFaction()]
                ...
                for k, v in ipairs(player.GetAll()) do
                    lia.util.notifyLocalized("cChangeFaction", v, client:Name(), target:Name(), L(faction.name, v))
                end
            end,
        ...
        lia.command.add("content", {
            onRun = function(client)
                client:SendLua([[gui.OpenURL(lia.config.get("contentURL", "https://lilia.net"))]])
            end
        })
    end
end)
