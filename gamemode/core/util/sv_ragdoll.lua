	...
	if (state) then
		...
		entity:CallOnRemove("fixer", function()
			if (IsValid(self)) then
				...
				self:SetLocalVelocity(
					IsValid(entity)
					and entity.liaLastVelocity
					or vector_origin
				)
			end

			if (IsValid(self) and !entity.liaIgnoreDelete) then
				...
				if (self:isStuck()) then
					...
					local positions = lia.util.findEmptySpace(
						self,
						{entity, self}
					)
		...
		if (time and time > 0) then
			...
			self:setAction(
				"@wakingUp",
				nil, nil,
				entity.liaStart, entity.liaFinish
			)
		end
