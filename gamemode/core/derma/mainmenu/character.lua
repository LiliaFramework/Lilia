...
function PANEL:createWelcomeScreen()
    ...
    local function formatPlaytimeText(playtime)
        ...
        elseif hours > 0 then
            duration = string.format("%d%s %d%s", hours, hourSuffix, minutes, minuteSuffix)
        else
    ...
    local steamName = getDisplayName()
    ...
    if not isFirstJoin then
        ...
        playtimeLabel:SetText(formatPlaytimeText(playtime))
    ...
    self.welcomeScreen.Think = function(pnl)
        if IsValid(client) then
            local currentSteamName = getDisplayName()
            if currentSteamName ~= lastSteamName and currentSteamName ~= defaultPlayerName then
...
function PANEL:createSelectedCharacterInfoPanel(character)
    ...
    for i, text in ipairs(info) do
        if i == 1 then
            ...
            countLabel:SetFont("LiliaFont.17")
