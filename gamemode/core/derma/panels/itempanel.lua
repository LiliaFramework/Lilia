...
function PANEL:Init()
    ...
    self.descLabel = vgui.Create("DLabel", self.header)
    self.descLabel:SetFont("liaBigText")
    self.descLabel:SetTextColor(color_white)
    self.descLabel:SetContentAlignment(5)
    self.descLabel:Dock(TOP)
    self.descLabel:DockMargin(0, 5, 0, 0)
...
function PANEL:openInspect()
    ...
    drawLine(scroll, L"Description", self.item:getDesc() or "")
    local extra = {}
...
function PANEL:SetEntity(ent)
    ...
    self.descLabel:SetText(self.item:getDesc() or "")
    self.descLabel:SizeToContentsY()
    self.header:SetTall(self.nameLabel:GetTall() + self.descLabel:GetTall() + 5)
    self:buildButtons()
end
