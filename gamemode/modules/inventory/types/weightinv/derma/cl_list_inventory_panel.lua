...
function PANEL:addStack(key, stack)
    if IsValid(self.icons[key]) or #stack == 0 then return end
...
local function getItemStacks(inventory)
    local stacks = {}
    local stack, key
    for _, item in SortedPairs(inventory:getItems()) do
        key = getItemStackKey(item)
        stack = stacks[key] or {}
        stack[#stack + 1] = item
        stacks[key] = stack
    end
...
function PANEL:populateItems()
    ...
    local stacks = getItemStacks(self.inventory)
