    ...
    if client:hasPrivilege("viewEntityTab") then
        table.insert(pages, {
            ...
            drawFunc = function(panel)
                ...
                entityList:AddColumn(L("entityClass"))
                ...
                local function populateEntities()
                    ...
                    for _, ent in ipairs(ents.GetAll()) do
                        if IsValid(ent) and not ent:IsPlayer() then
                            local creator = "N/A"
                            if ent.GetCreator and IsValid(ent:GetCreator()) then creator = ent:GetCreator():Name() end
                            local pos = ent:GetPos()
                            local posStr = string.format("%.1f, %.1f, %.1f", pos.x, pos.y, pos.z)
                            local model = ent:GetModel() or "N/A"
                            entityList:AddLine(ent:GetClass(), model, posStr, creator)
                        end
                    end
                end
                ...
                entityList.OnRowRightClick = function(_, _, line)
                        ...
                        if #posParts == 3 then
                            ...
                            for _, ent in ipairs(ents.FindInSphere(pos, 10)) do
                                if ent:GetClass() == entClass then
                                    client:SetPos(ent:GetPos() + ent:GetForward() * 50)
                                    break
                                end
                            end
                        end
                    ...
                    menu:AddOption(L("copyEntityInfo"), function()
                        local info = string.format("%s | %s | %s | %s", line:GetColumnText(1), line:GetColumnText(2), line:GetColumnText(3), line:GetColumnText(4))
                        SetClipboardText(info)
                        client:notifyLocalized("copiedToClipboard", info, L("entityInfo"))
                    end):SetIcon("icon16/page_copy.png")
