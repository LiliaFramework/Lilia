    ...
    function()
        ...
        if fraction > 0 then
            ...
            for k, v in ipairs(weapons) do
                if not weapons[currentIndex] then currentIndex = total end
    ...
    function(client, bind, pressed)
        ...
        if not client:InVehicle() and (not IsValid(weapon) or weapon:GetClass() ~= "weapon_physgun" or not client:KeyDown(IN_ATTACK)) then
            ...
            if bind:find("invprev") and pressed then
                index = index - 1
                if index < 1 then index = #client:GetWeapons() end
                onIndexChanged()
                return true
            elseif bind:find("invnext") and pressed then
                index = index + 1
                if index > #client:GetWeapons() then index = 1 end
                onIndexChanged()
                return true
