...
hook.Add(
    ...
    function(self, panel)
        if panel.liaToolTip or panel.itemID then
            self.markupObject = nut.markup.parse(self:GetText(), ScrW() * .15)
            self:SetText("")
...
hook.Add(
    ...
    function(self, w, h)
        if self.isItemTooltip then
            ...
            if self.markupObject then self.markupObject:draw(12 * 0.5, 12 * 0.5 + 2) end
            return true
        end
    end
)
...
--------------------------------------------------------------------------------------------------------
hook.Add("TooltipLayout", "liaItemTooltip", function(self) if self.isItemTooltip then return true end end)
--------------------------------------------------------------------------------------------------------
hook.Add(
    "StartChat",
    "StartChatTyping",
