...
function GM:CanPlayerInteractItem(client, action, item)
    ...
    if action == "equip" and hook.Run("CanPlayerEquipItem", client, item) then
        if lia.config.EquipDelay > 0 then
            ...
            timer.Create("EquipDelay." .. ply:SteamID64(), lia.config.EquipDelay, 1, function() if IsValid(client) then ply.EquipDelay = nil end end)
        else
            return true
        end
    end
    ...
    if action == "drop" and hook.Run("CanPlayerDropItem", client, item) then
        if lia.config.DropDelay > 0 then
            ...
            timer.Create("DropDelay." .. ply:SteamID64(), lia.config.DropDelay, 1, function() if IsValid(client) then ply.DropDelay = nil end end)
        else
            return true
        end
    end
    ...
    if action == "take" and not hook.Run("CanPlayerTakeItem", client, item) then
        if lia.config.TakeDelay > 0 then
            ...
            timer.Create("TakeDelay." .. ply:SteamID64(), lia.config.TakeDelay, 1, function() if IsValid(client) then ply.TakeDelay = nil end end)
        else
            return true
        end
    end

    if inventory and (inventory.isBag == true or inventory.isBank == true) then if lia.config.DisallowedBagForbiddenActions[action] then return false, "forbiddenActionStorage" end end
end
