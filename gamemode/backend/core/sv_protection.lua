...
function GM:OnEntityCreated(ent)
    if lia.config.DrawEntityShadows then ent:DrawShadow(false) end
...
function GM:PlayerSpawnedVehicle(ply, ent)
    ...
    if not ply:IsSuperAdmin() then ply.NextVehicleSpawn = SysTime() + delay end
end
...
function GM:PhysgunDrop(ply, ent)
    ...
    timer.Simple(5, function() if IsValid(ent) and ent:GetCollisionGroup() == COLLISION_GROUP_PASSABLE_DOOR then ent:SetCollisionGroup(COLLISION_GROUP_NONE) end end)
end