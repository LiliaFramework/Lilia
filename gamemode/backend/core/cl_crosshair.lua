...
function GM:PostDrawHUD()
    ...
    if client:ShouldDrawLocalPlayer() then cx, cy = screen.x, screen.y end
    if icon then
        if icon ~= lastIcon then lastIcon = icon end
    ...
    if lastIcon then lia.util.drawText(lastIcon or "", cx + (adx or 0), cy + (ady or 0), ColorAlpha(color_white, curIconAlpha), 1, 1, "liaCrossIcons") end
end
...
function GM:GetCrosshairIcon(curAlpha, entity, weapon, distance)
    ...
    if IsValid(wep) then if wep:GetNW2Bool("holdingObject", false) == true then return "" end end
