...
function playerMeta:setRestricted(state, noMessage)
    if state then
        ...
        if noMessage then self:setLocalVar("restrictNoMsg", true) end
        self.liaRestrictWeps = self.liaRestrictWeps or {}
        for k, v in ipairs(self:GetWeapons()) do
            self.liaRestrictWeps[k] = v:GetClass()
        end

        timer.Simple(0, function() self:StripWeapons() end)
        hook.Run("OnPlayerRestricted", self)
    else
        self:setNetVar("restricted")
        if self:getLocalVar("restrictNoMsg") then self:setLocalVar("restrictNoMsg") end
