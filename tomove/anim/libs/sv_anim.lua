...
function MODULE:VelocityIsHigher(ply, value)
...
function MODULE:ApplyAnimation(ply, targetValue, class)
    local classassigned
...
function MODULE:ToggleAnimaton(ply, crossing, class, deactivateOnMove)
    if crossing then
...
function MODULE:SetupMove(ply, moveData, cmd)
    if ply:GetNWBool("animationStatus") then
