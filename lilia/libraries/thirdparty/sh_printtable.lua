-- https://github.com/Be1zebub/Small-GLua-Things/blob/master/sh_printtable.lua
--[[
table.Print({
	"Hello world!",
	true,
...
function table.ToPlain(tbl, nopretty, lvl, already)
    ...
    for k, v in (isSeq and ipairs or pairs)(tbl) do
        if not_isSeq then k, v = v.k, v.v end
        if isSeq then
            k = ""
        ...
        if istable[type(v)] and already[v] == nil then
            ...
            last = v
            out[#out + 1] = k .. (isstring == getmetatable(v) and format("%q", v) or tostring(v))
        end
    end

    if len == 1 and istable[type(last)] == nil then return "{" .. out[1] .. "}" end
    if nopretty then
        return "{" .. concat(out, ", ") .. "}"
    else
