...
local metaTable_Vector2 = {
    __add = function(self, other) return Vector2(self.x + other.x, self.y + other.y) end,
    __sub = function(self, other) return Vector2(self.x - other.x, self.y - other.y) end,
    ...
    __div = function(self, other)
        if isnumber(other) then return Vector2(self.x / other, self.y / other) end
        return Vector2(self.x / other.x, self.y / other.y)
    end,
    __tostring = function(self) return string.format("%.6f %.6f", self.x, self.y) end,
    SetUnpacked = function(self, x, y)
        self.x = x
        self.y = y
    end,
    GetPos = function(self) return Vector2(self.x, self.y) end
}
...
function tween.LerpAngle(from, to, t)
    return Angle(LerpDeg(from.p, to.p, t), LerpDeg(from.y, to.y, t), LerpDeg(from.r, to.r, t))
end
...
function LerpColor(from, to, t)
    return Color(Lerp(from.r, to.r, t), Lerp(from.g, to.g, t), Lerp(from.b, to.b, t), Lerp(from.a, to.a, t))
end

local function LerpColorUnpacked(color, from, to, t)
    color:SetUnpacked(Lerp(from.r, to.r, t), Lerp(from.g, to.g, t), Lerp(from.b, to.b, t), Lerp(from.a, to.a, t))
end

local function LerpAngleUnpacked(angle, from, to, t)
    angle:SetUnpacked(LerpDeg(from.p, to.p, t), LerpDeg(from.y, to.y, t), LerpDeg(from.r, to.r, t))
end
...
local function tween_type(object)
    return isvector2(object) and "vector2" or IsColor(object) and "color" or type(object):lower()
end
...
local metaTable_Tween = {
    ...
    SetPermanent = function(self, bool) self.permanent = bool end,
    SetFrom = function(self, from) self.from = from end,
    SetTo = function(self, to) self.to = to end,
    SetWaypoints = function(self, from, to)
        self.from = from
        self.to = to
    end,
    SetDuration = function(self, duration) self.duration = duration end,
    SetEaseType = function(self, ease_type) self.ease_type = ease_type end,
    Restart = function(self)
        if not all_tweens[self] then
            self:Start()
            return
        end
        ...
        self.end_time = self.start_time + self.duration
        self.time_left = self.duration
        if not running_tweens[self] then running_tweens[self] = true end
    ...
    Update = function(self)
        if self.running then
            ...
            if OnUpdate and isfunction(OnUpdate) then self:OnUpdate() end
        end
    end,
    TimeLeft = function(self) return self.time_left end,
    ...
    SetCallback = function(self, callback) self.callback = callback end
}
