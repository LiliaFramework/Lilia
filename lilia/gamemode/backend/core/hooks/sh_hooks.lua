...
function GM:InitializedModules()
    ...
    timer.Simple(2, function() self:TPosingModelsFix() end)
end
...
function GM:PlayerSpawn(client)
    if SERVER then self:PlayerSpawnServer(client) end
end
...
function GM:PSALoader()
    if ulx or ULib then MsgC(Color(255, 0, 0), "WE DO NOT RECOMMEND THE USE OF ULX AS IT MAY CREATE PERFOMANCE ISSUES!" .. "\n") end
    if TalkModes then timer.Simple(2, function() MsgC(Color(255, 0, 0), "Please Remove Talk Modes. Our framework has such built in by default.") end) end
end
