...
do
	...
	function Service:loadMediaObject(media, url, opts)
		...
		self:resolveUrl(
			...
			function(resolvedUrl, resolvedData)
				...
				if resolvedData and resolvedData.start and (not opts or not opts.dontSeek) then
					media:seek(resolvedData.start)
				end
			end
		)
	end
	...
	function _service_cbchain_meta:run(err, data)
		...
		first(
			err,
			data,
			function(chainedErr, chainedData)
