...
do
    ...
    cvars.AddChangeCallback(cvar_debug:GetName(), function(_, _, val) medialib.DEBUG = val == "1" end)
    ...
    function medialib.module(name, opts)
        if medialib.DEBUG then print("[MediaLib] Creating module " .. name) end
    ...
    function medialib.tryInclude(file)
        ...
        if medialib.DEBUG then print("[MediaLib] Attempted to include nonexistent file " .. file) end
        return false
    end
    ...
    function medialib.load(name)
        ...
        if medialib.DEBUG then print("[MediaLib] Loading unreferenced module " .. name) end
    ...
    local real_file_meta = {
        ...
        load = function(self) return include(self.lua_path) end,
        --local str = self:read() --if not str then error("MedialibDynLoad: could not load " .. self.lua_path) end -- TODO this does not function correctly; -- embedded medialib loading real_file will use global medialib as its 'medialib' instance
        addcs = function(self) AddCSLuaFile(self.lua_path) end,
    }

    real_file_meta.__index = real_file_meta
    local virt_file_meta = {
        read = function(self) return self.source end,
        load = function(self)
...
do
    ...
    function oop.class(name, parent)
        ...
        if not cls then
            ...
            if medialib.DEBUG then print("[MediaLib] Registering oopclass " .. name) end
        end
        return cls
    end

    function oop.resolveClass(obj)
        if obj == nil then return oop.Object end
        local t = type(obj)
        if t == "string" then
    ...
    local metamethods = {'__add', '__call', '__concat', '__div', '__ipairs', '__le', '__len', '__lt', '__mod', '__mul', '__pairs', '__pow', '__sub', '__tostring', '__unm'}
    function oop.createClass(name, parent)
        local cls = {}
        -- Get parent class
        local par_cls
        if parent ~= NIL_PARENT then par_cls = oop.resolveClass(parent) end
        -- Add metadata
        cls.name = name
        cls.super = par_cls
        ...
        do
            ...
            class_meta.__tostring = function(self) return "class " .. self.name end
...
do
    ...
    function Media:sync(time, margin)
        ...
        if self._lastSync and self._lastSync > CurTime() - 5 then return end
    ...
    function Media:shouldSync(time, margin)
        ...
        if not self:isValid() or not self:isPlaying() then return false end
        margin = margin or 2
    ...
    function Media:runCommand()
    end

    function Media:draw()
    end

    function Media:getTag()
        return self._tag
    end

    function Media:setTag(tag)
        self._tag = tag
    end
...
do
    ...
    function media.guessService(url, opts)
        for name, service in pairs(media.Services) do
            local isViable = true
            if opts and opts.whitelist then isViable = isViable and table.HasValue(opts.whitelist, name) end
            if opts and opts.blacklist then isViable = isViable and not table.HasValue(opts.blacklist, name) end
            if isViable and service:isValidUrl(url) then return service end
        end
    end
...
do
    ...
    function Service:emit(event, ...)
        ...
        if event == "error" then MsgN("[MediaLib] Video error: " .. table.ToString{...}) end
    end
    ...
    function _service_cbchain_meta:run(err, data)
        ...
        first(err, data, function(chainedErr, chainedData) self:run(chainedErr, chainedData) end)
    end
...
do
    ...
    function TimeKeeper:getTime()
        ...
        if self.running then time = time + (RealTime() - self.runningTimeStart) end
        return time
    end
    ...
    function TimeKeeper:seek(time)
        ...
        if self.running then self.runningTimeStart = RealTime() end
    end
end
...
do
    ...
    function HTMLService:hasReliablePlaybackEvents()
        return false
    end
    ...
    function HTMLPool.get()
        if #HTMLPool.instances == 0 then
            if medialib.DEBUG then MsgN("[MediaLib] Returning new instance; htmlpool empty") end
            return HTMLPool.newInstance()
        end

        local inst = table.remove(HTMLPool.instances, 1)
        if not IsValid(inst) then
            if medialib.DEBUG then MsgN("[MediaLib] Returning new instance; instance was invalid") end
            return HTMLPool.newInstance()
        end

        if medialib.DEBUG then MsgN("[MediaLib] Returning an instance from the HTML pool") end
        return inst
    end
    ...
    function HTMLPool.free(inst)
        ...
        if #HTMLPool.instances >= GetMaxPoolInstances() then
            if medialib.DEBUG then MsgN("[MediaLib] HTMLPool full; removing the freed instance") end
            inst:Remove()
        else
            if medialib.DEBUG then MsgN("[MediaLib] Freeing an instance to the HTMLPool") end
            inst:SetHTML("")
    ...
    function HTMLMedia:initialize()
        ...
        pnl:AddFunction("medialiblua", "Event", function(id, jsonstr) self:handleHTMLEvent(id, util.JSONToTable(jsonstr)) end)
    end
    ...
    function HTMLMedia:handleHTMLEvent(id, event)
        if medialib.DEBUG then MsgN("[MediaLib] HTML Event: " .. id .. " (" .. table.ToString(event) .. ")") end
        if id == "stateChange" then
            local state = event.state
            local setToState
            if event.time then self.timeKeeper:seek(event.time) end
    ...
    function HTMLMedia:getHTMLMaterial()
        if self._htmlMat then return self._htmlMat end
    ...
    function HTMLMedia:setQuality(qual)
        if self.lastSetQuality and self.lastSetQuality == qual then return end
    ...
    function HTMLMedia:applyVolume()
        ...
        local vol = ivol * rvol
        if self.lastSetVolume and self.lastSetVolume == vol then return end
    ...
    function HTMLMedia:play()
        if not self:hasReliablePlaybackEvents() then self.timeKeeper:play() end
        self:runJS("medialibDelegate.run('play')")
    end

    function HTMLMedia:pause()
        if not self:hasReliablePlaybackEvents() then self.timeKeeper:pause() end
...
do
    ...
    function BASSMedia:openUrl(url)
        ...
        sound.PlayURL(url, flags, function(chan, errId, errName) self:bassCallback(chan, errId, errName) end)
    end

    function BASSMedia:openFile(path)
        self._openingInfo = {"file", path}
        local flags = table.concat(self.bassPlayOptions, " ")
        sound.PlayFile(path, flags, function(chan, errId, errName) self:bassCallback(chan, errId, errName) end)
    end
    ...
    function BASSMedia:reload()
        ...
        if self._commandState == "play" then self:play() end
    end
    ...
    function BASSMedia:applyVolume(force)
        ...
        local rvol = self.volume or 1
        local vol = ivol * rvol
        if not force and self.lastSetVolume and self.lastSetVolume == vol then return end
        self.lastSetVolume = vol
    ...
    function BASSMedia:getTime()
        if self:isValid() and IsValid(self.chan) then return self.chan:GetTime() end
        return 0
    end
...
do
    ...
    for _, file in medialib.folderIterator("services") do
        if medialib.DEBUG then print("[MediaLib] Registering service " .. file.name) end
