...
local all_patterns = {"^https?://www.soundcloud.com/([A-Za-z0-9_%-]+/[A-Za-z0-9_%-]+)/?.*$", "^https?://soundcloud.com/([A-Za-z0-9_%-]+/[A-Za-z0-9_%-]+)/?.*$",}
...
function SoundcloudService:resolveUrl(url, callback)
    ...
    if type(apiKey) == "table" then apiKey = table.Random(apiKey) end
