...
function ThirdPersonCore:CalcView(client, origin, angles, fov)
    ...
    if client:CanOverrideView() and LocalPlayer():GetViewEntity() == LocalPlayer() then
        ...
        view = {}
        traceData = {}
        traceData.start = client:GetPos() + client:GetViewOffset() + curAng:Up() * clmp(CVAR_TP_VERT:GetInt(), 0, 30) + curAng:Right() * clmp(CVAR_TP_HORI:GetInt(), -30, 30) - client:GetViewOffsetDucked() * .5 * crouchFactor
        traceData.endpos = traceData.start - curAng:Forward() * clmp(CVAR_TP_DIST:GetInt(), 0, 100)
