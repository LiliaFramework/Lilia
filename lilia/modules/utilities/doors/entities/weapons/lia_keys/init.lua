...
function SWEP:ServerSecondaryAttack(owner, entity, time)
    if (entity:isDoor() and entity:checkDoorAccess(owner)) or (entity:IsVehicle() or (simfphys and simfphys.IsCar(entity))) and (entity:GetCreator() == owner) then
        owner:setAction("@unlocking", time, function() self:ToggleLock(owner, entity, false) end)
        return
    end
end
...
function SWEP:ServerPrimaryAttack(owner, entity, time)
    if (entity:isDoor() and entity:checkDoorAccess(owner)) or (entity:IsVehicle() or (simfphys and simfphys.IsCar(entity))) and (entity:GetCreator() == owner) then
        owner:setAction("@locking", time, function() self:ToggleLock(owner, entity, true) end)
        return
    end
end
...
function SWEP:ToggleLock(owner, door, state)
    ...
    elseif door:IsVehicle() then
        if state then
            door:Fire("lock")
            if simfphys.IsCar(door) then door.IsLocked = true end
            owner:EmitSound("doors/door_latch3.wav")
        else
            door:Fire("unlock")
            if simfphys.IsCar(door) then door.IsLocked = false end
            owner:EmitSound("doors/door_latch1.wav")
        end
    end
end
