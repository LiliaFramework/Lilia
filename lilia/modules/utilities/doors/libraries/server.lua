...
local DarkRPVariables = {
    ["DarkRPNonOwnable"] = function(entity, _)
        entity:setNetVar("noSell", true)
    end,
    ["DarkRPTitle"] = function(entity, val)
        entity:setNetVar("name", val)
    end,
    ["DarkRPCanLockpick"] = function(entity, val)
        entity.noPick = tobool(val)
    end
}
...
function DoorsCore:KeyUnlock(client, entity, time)
    ...
    client:setAction(
        ...
        time,
        function()
            self:ToggleLock(client, entity, false)
        end
    )
