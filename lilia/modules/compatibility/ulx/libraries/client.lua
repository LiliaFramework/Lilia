...
function MODULE:OpenAdminStickUI(target)
    ...
    if IsValid(target) then
        if IsValid(target) and target:IsPlayer() then
            ...
            local returnDeath = teleport:AddOption("Return to Death Position", function()
                LocalPlayer():ConCommand("say /return")
                AdminStickIsOpen = false
            end)

            returnDeath:SetIcon("icon16/arrow_undo.png")
            -- Flags Management Submenu
        ...
        elseif target:isDoor() then
            ...
            if not target:getNetVar("hidden") then
                AdminMenu:AddOption("Make Door Hidden", function()
                    LocalPlayer():ConCommand('say /doorsethidden')
                    AdminStickIsOpen = false
                end):SetIcon("icon16/door.png")
            else
                AdminMenu:AddOption("Make Door Visible", function()
            ...
            if target:getNetVar("noSell") == true then
                ...
                AdminMenu:AddOption("Make Door Unownable", function()
                    LocalPlayer():ConCommand('say /doorsetunownable')
                    AdminStickIsOpen = false
                end):SetIcon("icon16/door_in.png")
            end

            local factionsAssigned = string.Split(target:getNetVar("factions", "") or "", ",")
            local addFactionMenu = AdminMenu:AddSubMenu("Add Faction to Door")
            for _, v in pairs(lia.faction.teams) do
                if not table.HasValue(factionsAssigned, tostring(v.uniqueID)) then
                    addFactionMenu:AddOption(v.name, function()
            ...
            if #factionsAssigned > 0 and factionsAssigned[1] ~= "" then
                local removeFactionMenu = AdminMenu:AddSubMenu("Remove Faction from Door")
                for _, v in pairs(lia.faction.teams) do
                    if table.HasValue(factionsAssigned, tostring(v.uniqueID)) then
                        removeFactionMenu:AddOption(v.name, function()
                ...
                AdminMenu:AddOption("Clear Factions from Door", function()
        ...
        elseif target:GetClass() ~= "worldspawn" then
            local entityOption = AdminMenu:AddOption("Get Entity Name", function()
                LocalPlayer():ConCommand("say /entityName")
                AdminStickIsOpen = false
            end)
