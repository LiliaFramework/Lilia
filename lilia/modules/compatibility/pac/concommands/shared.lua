...
function _G.pac_Restart()
    ...
    if pace then
        if pace.Editor and pace.Editor:IsValid() then
            ...
            if pace.current_part and pace.current_part:IsValid() then selected_part_uid = pace.current_part:GetUniqueID() end
            for _, part in pairs(pac.GetLocalParts()) do
                if not part:HasParent() and part:GetShowInEditor() then
                    local ok, err = pcall(function() table.insert(prev_parts, part:ToTable()) end)
    ...
    if pac and pac.Disable then
        ...
        for hook_name, hooks in pairs(hook.GetTable()) do
            for id, _ in pairs(hooks) do
                if isstring(id) and (id:StartWith("pace_") or id:StartWith("pac_") or id:StartWith("pac3_") or id:StartWith("pacx_")) then hook.Remove(hook_name, id) end
            end
        end
    ...
    if not prefer_local_version:GetBool() then
        ...
        for _, path in ipairs(file.Find("autorun/pac*", "LUA")) do
            if path:EndsWith("_init.lua") and path ~= "pac_init.lua" then include("autorun/" .. path) end
        end
    elseif sv_allowcslua:GetBool() or LocalPlayer():IsSuperAdmin() then
        local loadingHit = false
        if sv_allowcslua:GetBool() then pacLocal.Message("pac_restart: sv_allowcslua is on, looking for PAC3 addon..") end
        if LocalPlayer():IsSuperAdmin() then pacLocal.Message("pac_restart: LocalPlayer() is superadmin, looking for PAC3 addon..") end
            ...
            if file.Exists("addons/" .. dir .. "/lua/autorun/pac_editor_init.lua", "MOD") then
                ...
                local function include(path, ...)
                    ...
                    if not file.Exists("addons/" .. dir .. "/lua/" .. path, "MOD") then
                        ...
                        if lua_dir:StartWith("addons/" .. dir) then lua_dir = lua_dir:match("addons/.-/lua/(.+)") end
                ...
                for _, path in ipairs(file.Find("autorun/pac_*", "LUA")) do
                    if path:EndsWith("_init.lua") and path ~= "pac_init.lua" then
                        ...
                        local ok, err = pcall(function() include("autorun/" .. path) end)
                        if not ok then pacLocal.Message("pac_restart: error when reloading pac " .. err) end
                    end
                end
    ...
    _G.PAC_RESTART = nil
    if editor_was_open then pace.OpenEditor() end
    pac.Enable()
    if prev_parts[1] then pace.LoadPartsFromTable(prev_parts, true) end
    ...
    if selected_part_uid then
        ...
        if part and part:IsValid() then pace.Call("PartSelected", part) end
    end
