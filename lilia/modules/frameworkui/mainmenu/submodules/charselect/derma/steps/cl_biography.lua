...
function PANEL:addTextEntry(contextName)
    ...
    entry.onTabPressed = function()
        self.name:RequestFocus()
    end -- Fix for tab focus
...
function PANEL:addLabel(contextName)
    ...
    label:Dock(TOP) -- Dock labels to the top
    label:DockMargin(0, 8, 0, 0) -- Adjust margin as needed
    return label
end
...
function PANEL:onDisplay()
    ...
    if overrideName then
        ...
        if defaultName and not self:getContext("name") then
    ...
    if overrideDesc then
        ...
        if defaultDesc and not self:getContext("desc") then
