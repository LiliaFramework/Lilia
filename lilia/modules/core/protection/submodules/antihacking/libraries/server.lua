...
function net.Incoming(len, client)
    ...
    if antiNetSpam[plySteamid][name] > AntiHacking.threshold then
        if not flaggedNetPlayers[plySteamid] then
            ...
            file.Append("lilia/netlogs/" .. os.date("%x"):gsub("/", "-") .. ".txt", "[" .. os.date("%X") .. "]\t" .. string.format("Net spam attempted on Net Message: %s Client: %s (STEAMID: %s) (IP: %s) \n", name, plyNick, plySteamid, plyIP) .. "\r\n")
        end
    ...
    if not status then
        ...
        file.Append("lilia/netlogs/" .. os.date("%x"):gsub("/", "-") .. ".txt", "[" .. os.date("%X") .. "]\t" .. string.format("Error during net message (%s). Reasoning: %s \n", name, error) .. "\r\n")
    end
end
...
function concommand.Run(ply, cmd, args, argStr)
    ...
    if antiConSpam[plySteamid][cmd] > 10 then
        if not flaggedConPlayers[plySteamid] then
            ...
            file.Append("lilia/concommandlogs/" .. os.date("%x"):gsub("/", "-") .. ".txt", "[" .. os.date("%X") .. "]\t" .. "Player " .. "'" .. plyNick .. "'" .. " (" .. plySteamid .. ") " .. "(" .. plyIP .. ")" .. " has attempted to concommand spam with command: " .. cmd .. " args: " .. temp .. "\r\n")
        end
