...
function GM:CanTool(client, _, tool)
    ...
    if client:getChar():hasFlags("t") or client:isStaffOnDuty() and CAMI.PlayerHasAccess(client, privilege, nil) then
        ...
        if tool == "remover" and validEntity then
            if table.HasValue(RestrictionCore.RemoverBlockedEntities, entity:GetClass()) then
