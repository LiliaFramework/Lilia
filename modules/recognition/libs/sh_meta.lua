...
function MODULE:IsCharRecognized(char, id)
    ...
    if other then
        ...
        if lia.config.FactionAutoRecognize and char:getFaction() == other:getFaction() and not self.noRecognise[char:getFaction()] and not self.noRecognise[other:getFaction()] then return end
