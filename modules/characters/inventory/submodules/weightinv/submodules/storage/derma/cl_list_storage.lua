local PANEL = {}
function PANEL:Init()
	if IsValid(lia.gui.storage) then lia.gui.storage:Remove() end
	lia.gui.storage = self
	self.gridW = lia.config.get("invW")
	local width = self.gridW * (64 + 4)
	self:SetSize(width * 2 + 8, ScrH() * 0.6)
	self:MakePopup()
	self:Center()
...
function PANEL:OnRemove()
	...
	lia.module.list["storage"]:exitStorage()
end

function PANEL:setStorage(storage)
	if IsValid(self.storage) then self.storage:RemoveCallOnRemove("ListStorageView") end
	...
	self.storageInv.onItemPressed = function(panel, itemIcon, keyCode) self:onItemPressed(itemIcon.itemID) end
	...
	self.localInv.onItemPressed = function(panel, itemIcon, keyCode) self:onItemPressed(itemIcon.itemID) end
end
