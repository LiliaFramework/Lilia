...
function PANEL:Init()
	self.gridW = lia.config.get("invW", 5)
	self.gridH = lia.config.get("invH", 5)
	self:SetSize(self.gridW * (64 + PADDING) + BORDER_FIX_W, self.gridH * (64 + PADDING) + PADDING + HEADER_HEIGHT + BORDER_FIX_H + WEIGHT_PANEL_HEIGHT)
	self:MakePopup()
...
vgui.Register("liaListInventory", PANEL, "liaInventory")