...
function PANEL:addItem(item)
	...
	icon:SetSize((item.width or 1) * size - PADDING, (item.height or 1) * size - PADDING)
...
function PANEL:drawHeldItemRectangle()
	...
	if drawTarget then
		...
		surface.DrawRect(drawTarget.x * size, drawTarget.y * size, drawTarget.w * size - PADDING, drawTarget.h * size - PADDING)
	else
...
function PANEL:Paint(w, h)
	...
	for y = 0, self.gridH - 1 do
		for x = 0, self.gridW - 1 do
			surface.DrawRect(x * (size + PADDING), y * (size + PADDING), size, size)
		end
	end
