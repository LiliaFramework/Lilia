...
function PANEL:Init()
    ...
    self.right:SetImage('external_g/arrow_right.png')
    ...
    self.right.DoClick = function()
        ...
        self.slides[self.currentSlide]:MoveTo(-ScrW(), (ScrH() * .5) - (ScrH() * .7) / 2, .3, 0, -1, function(_, me)
            if me.Hidden then
                me:Hidden()
            end
        end)
        ...
        self.slides[self.currentSlide + 1]:MoveTo((ScrW() * .5) - (ScrW() * .7) / 2, (ScrH() * .5) - (ScrH() * .7) / 2, .3, 0, -1, function()
            self.movingSlide = false
    ...
    self.left:SetImage('external_g/arrow_left.png')
