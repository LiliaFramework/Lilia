...
--------------------------------------------------------------------------------------------------------
    lia.chat.register("ic", {
            ...
            radius = function() return lia.config.ChatRange end
        }
    )
--------------------------------------------------------------------------------------------------------
    lia.chat.register("me",{
            ...
            onCanHear = function()
                ...
                if not trace.Hit then if speaker:EyePos():Distance(listener:EyePos()) <= lia.config.ChatRange then return true end end
                return false
            end,
            prefix = {"/me", "/action"},
...
--------------------------------------------------------------------------------------------------------
    lia.chat.register("w",{
            ...
            radius = function() return lia.config.ChatRange * 0.25 end,
            prefix = {"/w", "/whisper"}
        }
    )
--------------------------------------------------------------------------------------------------------
    lia.chat.register("y",{
            ...
            radius = function() return lia.config.ChatRange * 2 end,
            prefix = {"/y", "/yell"}
        }
    )
--------------------------------------------------------------------------------------------------------
    lia.chat.register("looc",{
            ...
            onChatAdd = function(speaker, text) chat.AddText(lia.chat.timestamp(false), Color(255, 50, 50), "[LOOC] ", lia.config.ChatColor, speaker:Name() .. ": " .. text) end,
            radius = function() return lia.config.ChatRange end,
            prefix = {".//", "[[", "/looc"},
            noSpaceAfter = true,
            filter = "ooc"
...
--------------------------------------------------------------------------------------------------------
    lia.chat.register("event",{
            ...
            onChatAdd = function(speaker, text) chat.AddText(lia.chat.timestamp(false), Color(255, 150, 0), text) end,
            prefix = {"/event"}
        }
    )
--------------------------------------------------------------------------------------------------------
